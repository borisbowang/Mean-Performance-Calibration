...
# arrival rate = 1
arrv <- rep(1,M)
target <- targetutl^2/(1-targetutl)

# set true utilization of different alternatives
utl <- seq(0.2,0.95,len=M)
# service rates
srv <- arrv/utl

# # set service rate
# srv <- seq(2, 1.05, len=M)
# utl <- arrv/srv

# true mean waiting times
wt_m <- utl^2/(arrv*(1-utl))
# warmup
warmup <- rep(200,M)
...
#2 setting prior
#########################################################################
trupar <- rbind(arrv, srv, warmup)
q <- tau <- rep(1,M)
al <- a <- rep(3/2,M)

# true (long-run estimation) Rr
